...
def knapsack01_dp(items, weight):
    table = [[0 for w in range(weight + 1)] for j in range(len(items) + 1)]
 
    for j in range(1, len(items) + 1):
        # gets the first item
        item, cls, wt, cost, val = items[j-1]
        # iterates through every integer up to weight
        for w in range(1, weight + 1):
            # if the weight of the item is greater than current integer
            if wt > w:
                table[j][w] = table[j-1][w]
                # else take the max between keeping old value or adding new item and its value
            else:
    ...
    for j in range(len(items), 0, -1):
        #was_added = boolean
        #if updated, that means we added and these two things do not equal each other
        was_added = table[j][w] != table[j-1][w]
 
        if was_added:
            item, cls, wt, cost, val = items[j-1]
            result.append(items[j-1])
          # see what the best item was at this weight
            w -= wt
...
def solve(P, M, N, C, items, constraints):
  ...
  #maxiset will return list of valid classes = (0, 1, 2, 5, 7)
  return maxiset
